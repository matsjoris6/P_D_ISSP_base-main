...
fs_sim = 44100
c = 343.0
d_ear = 0.013    # 1.3 cm (inter-mic afstand per oor)
d_front = 0.215  # 21.5 cm (L-R afstand)
...
# Kies luidsprekerpositie (zelfde als Week 2 Part 5)
folder_left = "s-30"   # bron links van de luisteraar
folder_right = "s30"   # bron rechts van de luisteraar

angle_L = abs(float(folder_left.replace("s", "")))
angle_R = float(folder_right.replace("s", ""))

# Ware hoeken in het MUSIC-kader (0°=boven, 90°=recht voor, 180°=onder)
true_front_R = 90.0 - angle_R
true_front_L = 90.0 + angle_L

# ==========================================
# 2. AUDIO EN RIR LADEN
# ==========================================
print("--- Audio en RIRs laden ---")
...
# Convolueer droge signalen met RIRs
...
# Mixsignalen (beide bronnen gecombineerd)
...
# Afzonderlijke spraakcomponenten (voor SNR-berekening)
# Bron 1 = doelbron (linker), Bron 2 = interferentie (rechter)
speech_L1 = yL1_from_left  # Bron 1 in linker mic 1
speech_L2 = yL2_from_left  # Bron 1 in linker mic 2
speech_R1 = yR1_from_left  # Bron 1 in rechter mic 1
speech_R2 = yR2_from_left  # Bron 1 in rechter mic 2

noise_L1  = yL1_from_right
noise_L2  = yL2_from_right
noise_R1  = yR1_from_right
noise_R2  = yR2_from_right

print("Audio geladen!")

# ==========================================
# 3. WIDEBAND MUSIC (2-MIC, 2D POSITIES)
# ==========================================
def music_wideband_2mics(micsigs, fs, mics_pos_2d, Q=1):
    """
    Wideband MUSIC voor een 2-microfoon array.
    mics_pos_2d : np.array van vorm [2, 2] met (x, y) per microfoon (gecentreerd).
                  Oor-array  → mics liggen langs y-as: bv. [0, +d/2] en [0, -d/2]
                  Frontaal   → mics liggen langs x-as: bv. [-d/2, 0] en [+d/2, 0]
    
    Convention (consistent met de rest van de code):
        θ = 0°   → boven (end-fire boven)
        θ = 90°  → recht voor (broadside)
        θ = 180° → onder (end-fire onder)
    TDOA = (px·cos θ + py·sin θ) / c   (zie Week 2 Part 5 / 4-mic versie)
    """
    L = 1024
    overlap = L // 2
    ...
    for m in range(micsigs.shape[1]):
        freqs, _, Zxx = signal.stft(micsigs[:, m], fs=fs, window='hann',
                                     nperseg=L, noverlap=overlap)
        stft_list.append(Zxx)
    stft_data = np.stack(stft_list, axis=0)  # [M x nF x nT]

    M, nF, nT = stft_data.shape
    angles = np.arange(0, 180.5, 0.5)
    rads   = np.radians(angles)

    # 2D posities (gecentreerd meegegeven)
    px = mics_pos_2d[:, 0].reshape(-1, 1)  # [M x 1]
    py = mics_pos_2d[:, 1].reshape(-1, 1)  # [M x 1]
    ...
    for k in range(1, L // 2):
        ...
        # TDOA: τ = (px·cos θ + py·sin θ) / c
        taus = (px * np.cos(rads) + py * np.sin(rads)) / c  # [M x nAngles]
        A    = np.exp(1j * omega * taus)                      # [M x nAngles]

        denom = np.sum(np.abs(En.conj().T @ A) ** 2, axis=0)
        pseudospectra.append(1.0 / (denom + 1e-10))
...
# ==========================================
# 4. WIDEBAND MUSIC (4-MIC, HEAD-MOUNTED)
# ==========================================
def music_wideband_4mics_hm(micsigs, fs, Q=2):
    """
    Wideband MUSIC voor het 4-microfoon hoofd-gemonteerde array.
    Microfoonposities: yL1, yL2 (linker oor), yR1, yR2 (rechter oor).
    """
    L = 1024
    overlap = L // 2
    ...
    x_ear = d_front / 2   # 10.75 cm (halve L-R afstand)
    y_mic = d_ear   / 2   # 0.65 cm  (halve inter-mic per oor)

    # Posities: [yL1, yL2, yR1, yR2] (x = links-rechts, y = voor-achter)
    mics_pos = np.array([
        [-x_ear,  y_mic],   # yL1
        [-x_ear, -y_mic],   # yL2
        [ x_ear,  y_mic],   # yR1
...
def das_bf_hm(micsigs, speech_comp, noise_comp, fs, mics_pos, target_doa_deg):
    ...
    px = mics_pos[:, 0]
    py = mics_pos[:, 1]
    # Zelfde TDOA-formule als in de MUSIC-functie: τ = (px·cosθ + py·sinθ) / c
...
# --- Microfoonposities voor 2-mic arrays (gecentreerd) ---
# Oor-arrays: microfoons liggen langs de y-as (voor-achter op het hoofd)
pos_right_2mic = np.array([[0,  y_mic],   # yR1
                            [0, -y_mic]])  # yR2
pos_left_2mic  = np.array([[0,  y_mic],   # yL1
                            [0, -y_mic]])  # yL2

# Frontaal: microfoons liggen langs de x-as (links-rechts)
pos_front_2mic = np.array([[-x_ear, 0],   # yL1
                            [ x_ear, 0]])  # yR1

# Rechter oor (Q=1: schat enkel de dominante richting)
_, _, est_R = music_wideband_2mics(micsigs_right, fs_sim, pos_right_2mic, Q=1)
target_R    = est_R[0]
print(f"Rechter oor – geschatte DOA: {target_R:.1f}° (waarde: {true_front_R}°)")

# Linker oor
_, _, est_L = music_wideband_2mics(micsigs_left, fs_sim, pos_left_2mic, Q=1)
target_L    = est_L[0]
print(f"Linker oor  – geschatte DOA: {target_L:.1f}° (waarde: {true_front_L}°)")

# Frontale mics
_, _, est_F = music_wideband_2mics(micsigs_front, fs_sim, pos_front_2mic, Q=1)
target_F    = est_F[0]
