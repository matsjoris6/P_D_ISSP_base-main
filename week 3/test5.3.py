...
try:
    ...
    # Speech = target (dry_sig_1 from folder_left), Noise = interferer (dry_sig_2 from folder_right)
    speech_L1 = yL1_from_left
    speech_L2 = yL2_from_left
    speech_R1 = yR1_from_left
    speech_R2 = yR2_from_left
...
def das_bf_hm(micsigs, speech_comp, noise_comp, fs, mics_pos, target_doa_deg):
    ...
    def apply_das(sigs, delays):
        ...
        for m in range(M_val):
            ...
            S_shifted = S * np.exp(1j * 2 * np.pi * freqs_fft * delays[m])
...
pos_L1 = np.array([-x_ear,  y_mic])
pos_L2 = np.array([-x_ear, -y_mic])
pos_R1 = np.array([ x_ear,  y_mic])
pos_R2 = np.array([ x_ear, -y_mic])
...
target_R    = est_R[0]
print(f"Rechter oor   – geschatte DOA: {target_R:.1f}° (verwacht: {true_front_R}°)")

_, _, est_L = music_wideband_2mics(micsigs_left, fs_sim, d_ear, Q=1)
target_L    = est_L[0]
print(f"Linker oor    – geschatte DOA: {target_L:.1f}° (verwacht: {true_front_L}°)")

_, _, est_F = music_wideband_2mics(micsigs_front, fs_sim, d_front, Q=1)
target_F    = est_F[0]
print(f"Frontale mics – geschatte DOA: {target_F:.1f}°")

_, _, est_4 = MUSIC_wideband_HM(micsigs_all4, fs_sim, Q=2)
target_4    = est_4[np.argmin(np.abs(est_4 - 90.0))]
print(f"4-Mic array   – geschatte DOAs: {est_4}, doelbron: {target_4:.1f}°")

# ==========================================
# 10. GSC PER CONFIGURATIE
# ==========================================

pos_right_c   = np.array([pos_R1, pos_R2])
pos_right_c  -= np.mean(pos_right_c, axis=0)
...
pos_left_c    = np.array([pos_L1, pos_L2])
pos_left_c   -= np.mean(pos_left_c, axis=0)
...
pos_frontal_c  = np.array([pos_L1, pos_R1])
pos_frontal_c -= np.mean(pos_frontal_c, axis=0)
...
pos_all4_c    = np.array([pos_L1, pos_L2, pos_R1, pos_R2])
pos_all4_c   -= np.mean(pos_all4_c, axis=0)
GSCout_4, SNR_4, DASout_4, SNR_DAS_4, noise_refs_4 = gsc_hm(
    micsigs_all4, speech_all4, noise_all4,
    fs_sim, pos_all4_c, target_4,
    label=f"4-Microfoon Array – Doelbron @ {target_4:.1f}°"
)

# ==========================================
# 11. VERGELIJKINGSPLOT
# ==========================================
labels  = ['Rechter Oor\n(yR1,yR2)', 'Linker Oor\n(yL1,yL2)',
           'Frontaal\n(yL1,yR1)', '4-Mic Array\n(alle)']
...
print("-" * 55)
configs = [
    ("Rechter Oor (yR1,yR2)",  SNR_DAS_R, SNR_R),
...
for name, das_snr, gsc_snr in configs:
    print(f"{name:<30} {das_snr:>9.1f} dB {gsc_snr:>9.1f} dB {gsc_snr - das_snr:>+11.1f} dB")
print("=" * 55)
