...
if __name__ == "__main__":
    ...
    for use_reverb in [False, True]:
        ...
        for i, rir_file in enumerate(rir_files):
            ...
            print(f"   [TIME] Verwerkingstijd GSC: {elapsed_time:.4f} seconden")
            
            # Optioneel: Real-time factor berekenen (RTF)
            # Als RTF < 1, is je filter sneller dan de werkelijke tijd (real-time capabel)
            rtf = elapsed_time / 10.0 # 10.0 is de segmentduur
            print(f"   [TIME] Real-time factor (RTF): {rtf:.4f}")
