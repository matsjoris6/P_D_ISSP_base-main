...
def music_wideband(micsigs, fs, acoustic_scenario):
    ...
    Since we only have 5 measured RIRs (one per source position) and not a dense
    angular grid covering all 1801 scan directions, a full replacement is not
    feasible here.  g(ω,θ) is therefore used for MUSIC; h(ω,θ) is used
    exclusively in the FAS BF look-up table inside gsc_fd.
    """
    # L=1024 met sqrt-Hanning venster: zelfde parameters als gsc_fd zodat de STFT-data
    # van de DOA-schatting rechtstreeks hergebruikt kan worden in het GSC-filter (cf. opdracht).
    L = 1024
    overlap = L // 2
    ...
    # In Python (0-based) is k=1 (DC) index 0. L/2 is index 512 (Nyquist for L=1024).
    ...
    for k in valid_indices:
        ...
        Y = stft_data[:, k, :]
        Ryy = (Y @ Y.conj().T) / nT
        # Diagonal loading: verbetert conditionering van Ryy in reverberante omgevingen.
        # Waarde 1e-3 is een goede startwaarde; verhoog naar 1e-2 bij sterke galm.
        diag_load = 1e-2 * np.trace(Ryy).real / M
        ...
        # Steering vector g(w,theta): free-field / geometric
...
def gsc_fd(scenario, speech_paths=None, noise_paths=None, duration=10.0, mu=0.05,
    ...
    # L en venster identiek aan music_wideband zodat de STFT-data gedeeld wordt (cf. opdracht).
    L = 1024
    overlap = L // 2
    window_sqrthann = np.sqrt(signal.windows.hann(L, sym=False))

    # Gebruik de DOA-geselecteerde target RIR indien meegegeven, anders fallback op source 0.
    ...
    # Gebruik de precomputed STFT van music_wideband indien beschikbaar (analyse-deel
    # dient twee doelen: DOA-schatting EN GSC, zie opdracht Part 1).
    if precomputed_stft_mix is not None:
        Zxx_mix = precomputed_stft_mix   # shape: (M, nF, nT)
    else:
...
if __name__ == "__main__":
    ...
    for use_reverb in [False, True]:
        ...
        for i, rir_file in enumerate(rir_files):
            ...
            # Echte DOAs berekenen via de bestaande hulpfunctie uit package/utils.py.
            gt_doas = calculate_ground_truth_doas(scenario)
            true_doa_tar = gt_doas[0]
            # interferer zit in noisePos; maak een tijdelijk scenario-achtig object om
            # dezelfde functie te hergebruiken
            ...
            # DOA-schatting met vrij-veld stuurvectoren g(w,θ).
            # De STFT van mic_mix wordt gereturnd en HERGEBRUIKT in gsc_fd
            # (analyse-deel dient twee doelen: DOA + GSC, cf. opdracht Part 1).
            # h(ω,θ) wordt NIET gebruikt in MUSIC: daarvoor zouden RIRs voor alle
            # 1801 scanhoeken nodig zijn, wat hier niet het geval is.
            ...
            # Koppel de geschatte DOA aan de juiste RIR voor de FAS BF LUT.
            # De target is de bron met de grootste hoek (links, 90°-180°).
            # We vergelijken de geschatte target-DOA met de ware DOAs van beide bronnen;
            # de dichtstbijzijnde bron wint en levert de RIR voor het GSC-filter.
            est_doas_sorted = np.sort(est_doas)
            # Grootste geschatte hoek = target (links van broadside)
            est_target_doa = est_doas_sorted[-1]
            dist_to_tar = abs(est_target_doa - true_doa_tar)
            dist_to_int = abs(est_target_doa - true_doa_int)
            if dist_to_tar <= dist_to_int:
                gsc_target_rir = target_rir   # correct: MUSIC vond de target
                print(f"   [DOA→RIR] Geschatte target-DOA ({est_target_doa:.1f}°) → target RIR geselecteerd")
            else:
                gsc_target_rir = interf_rir   # MUSIC verwisselde target/interferer
                print(f"   [DOA→RIR] Geschatte target-DOA ({est_target_doa:.1f}°) → interferer RIR geselecteerd (MUSIC verwisseld!)")

            # Bereken STFT van target en interferer met dezelfde L en venster als music_wideband.
            # Enkel nodig voor de SIR-evaluatie, niet voor het filter zelf.
            ...
            # Zo wordt de analyse-STFT maar EEN keer berekend (cf. opdracht).
            # De DOA-geselecteerde target RIR wordt expliciet meegegeven.
